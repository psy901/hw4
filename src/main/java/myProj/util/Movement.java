...
public class Movement extends SimpleAction implements FullActionModel {
	...
	protected int[] moveResult(int curX, int curY, int direction) {
		...
		// first get change in x and y from direction using 0: north; 1: south;
		// 2:east; 3: west
		int xdelta = 0;
		int ydelta = 0;
		if (direction == 0) {
		...
		if (nx < 0 || nx >= width || ny < 0 || ny >= height || this.map[nx][ny] == 1) {
			nx = curX;
			ny = curY;
		}
	...
	public List<TransitionProbability> getTransitions(State s, GroundedAction groundedAction) {
		...
		for (int i = 0; i < this.directionProbs.length; i++) {
			...
			if (newPos[0] != curX || newPos[1] != curY) {
				...
				// create transition probability object and add to our list of
				// outcomes
